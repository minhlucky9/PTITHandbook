...
public class JamesInteraction_QuizVP_MotCua : MonoBehaviour
{
    public QuizVP_MotCua QuizVP_MotCua;
            ...
            foreach (var key in GlobalResponseData.quests.Keys)
            {
                ...
                if (key == "QuestQuizVP_MotCuaSO")
                {
                    ...
                    if (quest.currentQuestStepIndex >= 0 && quest.currentQuestStepIndex < quest.questStepStates.Length)
                    {
                        ...
                        {
                            QuizVP_MotCua.score = stateValue;
                            Debug.Log($"QuizA1.score updated to: {QuizVP_MotCua.score} based on QuestStepState.state");
                        }
                        else
                        {
 ...
 //   private bool questStarted = false;
    ...
    void YourMethod()
    {
        ...
        {
            QuizA2Slider.value = QuizVP_MotCua.GetScore();
            QuizA2QuestLog.text = QuizVP_MotCua.GetScore().ToString() + "/8";
        }
    }
    ...
    private void QuestStateChange(Quest quest)
    {
        if (quest.info.id == questInfoForJames.id)
        {
            ...
            else if (quest.state == QuestState.FAILED)
            {
                ...
                uiFailedTimerUI.SetActive(true);
                questCompleted = false;
                QuizVP_MotCua.RestartGame();   
                QuizVP_MotCua.isTimerRunning = false;
    ...
    //    questProgressUI.SetActive(true);
        ...
        QuizVP_MotCua.isTimerRunning = true;
        isTimerRunning = false;
    }
