...
public class JamesInteraction_QuizA1 : MonoBehaviour
{
    public QuizA1 QuizA1;
    ...
    public QuestPoint QuizA1Point;
    private bool playerIsNear = false;


    [Header("QuestLog")]
    public Slider QuizA1Slider;
    public TextMeshProUGUI QuizA1QuestLog;
    ...
    private void Update()
    {
        ...
        if ((QuizA1Point.currentQuestState.Equals(QuestState.FINISHED)) && GlobalResponseData.quests != null)
        {
            //questCompleted = true;
            ...
            foreach (var key in GlobalResponseData.quests.Keys)
            {
                ...
                if (key == "QuestQuizA1SO")
                {
                    ...
                    if (quest.currentQuestStepIndex >= 0 && quest.currentQuestStepIndex < quest.questStepStates.Length)
                    {
                        ...
                        {
                            QuizA1.score = stateValue;
                            Debug.Log($"QuizA1.score updated to: {QuizA1.score} based on QuestStepState.state");
                        }
                        else
                        {
        ...
        else if (QuizA1Point.currentQuestState.Equals(QuestState.CAN_START))
        {
            questStartUI.SetActive(true);
        }
        else if (QuizA1Point.currentQuestState.Equals(QuestState.FAILED))
        {
            
        }
        Debug.Log(QuizA1Point.currentQuestState);
    }
 ...
 //   private bool questStarted = false;
    ...
    private void Start()
    {
        ...
        if (QuizA1Point.currentQuestState.Equals(QuestState.FAILED))
        {
    ...
    void YourMethod()
    {
        if (QuizA1Point.currentQuestState.Equals(QuestState.FINISHED))
        {
            QuizA1Slider.value = QuizA1.GetScore();
            QuizA1QuestLog.text = QuizA1.GetScore().ToString() + "/8";
        }
    }
    ...
    private void CompleteQuest()
    {
        if (questCompleted)
        {
          ...
          //  questCompleteUI.SetActive(true);
            ...
            inventoryData.AddItem(itemToPurchase, 1);
