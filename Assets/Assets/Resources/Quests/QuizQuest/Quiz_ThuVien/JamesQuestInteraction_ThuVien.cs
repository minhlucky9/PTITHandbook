...
public class JamesInteraction_Quiz_ThuVien : MonoBehaviour
{
    public Quiz_ThuVien Quiz_ThuVien;
    ...
    [Header("QuestLog")]
    public Slider Quiz_ThuVienSlider;
    public TextMeshProUGUI Quiz_ThuVienQuestLog;
            ...
            foreach (var key in GlobalResponseData.quests.Keys)
            {
                ...
                if (key == "QuestQuiz_ThuVienSO")
                {
                    ...
                    if (quest.currentQuestStepIndex >= 0 && quest.currentQuestStepIndex < quest.questStepStates.Length)
                    {
                        ...
                        {
                            Quiz_ThuVien.score = stateValue;
                            Debug.Log($"QuizA1.score updated to: {Quiz_ThuVien.score} based on QuestStepState.state");
                        }
                        else
                        {
 ...
 //   private bool questStarted = false;
    ...
    void YourMethod()
    {
        ...
        {
            Quiz_ThuVienSlider.value = Quiz_ThuVien.GetScore();
            Quiz_ThuVienQuestLog.text = Quiz_ThuVien.GetScore().ToString() + "/8";
        }
    }
    ...
    private void QuestStateChange(Quest quest)
    {
        if (quest.info.id == questInfoForJames.id)
        {
            ...
            else if (quest.state == QuestState.FAILED)
            {
                ...
                uiFailedTimerUI.SetActive(true);
                questCompleted = false;
                Quiz_ThuVien.RestartGame();   
                Quiz_ThuVien.isTimerRunning = false;
    ...
    //    questProgressUI.SetActive(true);
        ...
        Quiz_ThuVien.isTimerRunning = true;
        isTimerRunning = false;
    }
