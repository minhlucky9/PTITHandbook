...
public class JamesInteraction_QuizA1_2 : MonoBehaviour
{
    public QuizA1_2 QuizA1_2;
    ...
    public QuestPoint QuizA1_2Point;
    private bool playerIsNear = false;

    [Header("QuestLog")]
    public Slider QuizA1_2Slider;
    public TextMeshProUGUI QuizA1_2QuestLog;
    ...
    private void Update()
    {
        ...
        if ((QuizA1_2Point.currentQuestState.Equals(QuestState.FINISHED)) && GlobalResponseData.quests != null)
        {
            //questCompleted = true;
            ...
            foreach (var key in GlobalResponseData.quests.Keys)
            {
                ...
                if (key == "QuestQuizA1-2SO")
                {
                    ...
                    if (quest.currentQuestStepIndex >= 0 && quest.currentQuestStepIndex < quest.questStepStates.Length)
                    {
                        ...
                        {
                            QuizA1_2.score = stateValue;
                            Debug.Log($"QuizA1.score updated to: {QuizA1_2.score} based on QuestStepState.state");
                        }
                        else
                        {
        ...
        else if (QuizA1_2Point.currentQuestState.Equals(QuestState.CAN_START))
        {
            questStartUI.SetActive(true);
        }
        else if (QuizA1_2Point.currentQuestState.Equals(QuestState.FAILED))
        {
            
        }
        Debug.Log(QuizA1_2Point.currentQuestState);
    }
 ...
 //   private bool questStarted = false;
    ...
    void YourMethod()
    {
        if (QuizA1_2Point.currentQuestState.Equals(QuestState.FAILED))
        {
            uiFailedTimerUI.SetActive(true);
            failTimer = 300f;
            GameEventsManager.instance.questEvents.CancelQuest(questInfoForJames.id);
        }
        if (QuizA1_2Point.currentQuestState.Equals(QuestState.FINISHED))
        {
            QuizA1_2Slider.value = QuizA1_2.GetScore();
            QuizA1_2QuestLog.text = QuizA1_2.GetScore().ToString() + "/8";
        }
    }
    ...
    private void QuestStateChange(Quest quest)
    {
        if (quest.info.id == questInfoForJames.id)
        {
            ...
            else if (quest.state == QuestState.FAILED)
            {
                ...
                uiFailedTimerUI.SetActive(true);
                questCompleted = false;
                QuizA1_2.RestartGame();   
                QuizA1_2.isTimerRunning = false;
