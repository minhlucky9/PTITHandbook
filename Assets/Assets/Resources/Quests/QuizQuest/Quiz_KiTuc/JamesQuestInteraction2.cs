...
public class JamesInteraction_QuizKiTuc : MonoBehaviour
{
    public Quiz_KiTuc Quiz_KiTuc;
    ...
    [Header("QuestLog")]
    public Slider Quiz_KiTucSlider;
    public TextMeshProUGUI Quiz_KiTucQuestLog;
            ...
            foreach (var key in GlobalResponseData.quests.Keys)
            {
                ...
                if (key == "QuestQuiz_KiTucSO")
                {
                    ...
                    if (quest.currentQuestStepIndex >= 0 && quest.currentQuestStepIndex < quest.questStepStates.Length)
                    {
                        ...
                        {
                            Quiz_KiTuc.score = stateValue;
                            Debug.Log($"QuizA1.score updated to: {Quiz_KiTuc.score} based on QuestStepState.state");
                        }
                        else
                        {
    ...
    void YourMethod()
    {
        ...
        {
            Quiz_KiTucSlider.value = Quiz_KiTuc.GetScore();
            Quiz_KiTucQuestLog.text = Quiz_KiTuc.GetScore().ToString() + "/8";
        }
    }
    ...
    private void QuestStateChange(Quest quest)
    {
        if (quest.info.id == questInfoForJames.id)
        {
            ...
            else if (quest.state == QuestState.FAILED)
            {
                ...
                uiFailedTimerUI.SetActive(true);
                questCompleted = false;
                Quiz_KiTuc.RestartGame();
                Quiz_KiTuc.isTimerRunning = false;
    ...
    public void StartQuest()
    {
        ...
        Quiz_KiTuc.isTimerRunning = true;
        isTimerRunning = false;
    }
