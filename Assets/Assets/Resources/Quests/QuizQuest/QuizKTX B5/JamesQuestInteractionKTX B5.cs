...
public class JamesInteraction_QuizKTX_B5 : MonoBehaviour
{
    public QuizKTX_B5 QuizKTX_B5;
            ...
            foreach (var key in GlobalResponseData.quests.Keys)
            {
                ...
                if (key == "QuestQuizKTX_B5SO")
                {
                    ...
                    if (quest.currentQuestStepIndex >= 0 && quest.currentQuestStepIndex < quest.questStepStates.Length)
                    {
                        ...
                        {
                            QuizKTX_B5.score = stateValue;
                            Debug.Log($"QuizA1.score updated to: {QuizKTX_B5.score} based on QuestStepState.state");
                        }
                        else
                        {
 ...
 //   private bool questStarted = false;
    ...
    void YourMethod()
    {
        ...
        {
            QuizA2Slider.value = QuizKTX_B5.GetScore();
            QuizA2QuestLog.text = QuizKTX_B5.GetScore().ToString() + "/8";
        }
    }
    ...
    private void QuestStateChange(Quest quest)
    {
        if (quest.info.id == questInfoForJames.id)
        {
            ...
            else if (quest.state == QuestState.FAILED)
            {
                ...
                uiFailedTimerUI.SetActive(true);
                questCompleted = false;
                QuizKTX_B5.RestartGame();
                QuizKTX_B5.isTimerRunning = false;
    ...
    //    questProgressUI.SetActive(true);
        ...
        QuizKTX_B5.isTimerRunning = true;
        isTimerRunning = false;
    }
