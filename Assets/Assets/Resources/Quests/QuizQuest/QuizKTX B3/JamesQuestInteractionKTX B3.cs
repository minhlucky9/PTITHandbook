...
public class JamesInteraction_QuizKTX_B3 : MonoBehaviour
{
    public QuizKTX_B3 QuizKTX_B3;
    ...
    [Header("QuestLog")]
    public Slider QuizKTX_B3Slider;
    public TextMeshProUGUI QuizKTX_B3QuestLog;
            ...
            foreach (var key in GlobalResponseData.quests.Keys)
            {
                ...
                if (key == "QuestQuizKTX_B3SO")
                {
                    ...
                    if (quest.currentQuestStepIndex >= 0 && quest.currentQuestStepIndex < quest.questStepStates.Length)
                    {
                        ...
                        {
                            QuizKTX_B3.score = stateValue;
                            Debug.Log($"QuizA1.score updated to: {QuizKTX_B3.score} based on QuestStepState.state");
                        }
                        else
                        {
 ...
 //   private bool questStarted = false;
    ...
    void YourMethod()
    {
        ...
        {
            QuizKTX_B3Slider.value = QuizKTX_B3.GetScore();
            QuizKTX_B3QuestLog.text = QuizKTX_B3.GetScore().ToString() + "/8";
        }
    }
    ...
    private void QuestStateChange(Quest quest)
    {
        if (quest.info.id == questInfoForJames.id)
        {
            ...
            else if (quest.state == QuestState.FAILED)
            {
                ...
                uiFailedTimerUI.SetActive(true);
                questCompleted = false;
                QuizKTX_B3.RestartGame();   
                QuizKTX_B3.isTimerRunning = false;
    ...
    //    questProgressUI.SetActive(true);
        ...
        QuizKTX_B3.isTimerRunning = true;
        isTimerRunning = false;
    }
