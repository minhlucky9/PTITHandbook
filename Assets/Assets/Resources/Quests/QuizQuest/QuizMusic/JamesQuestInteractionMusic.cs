...
public class JamesInteraction_QuizMusic : MonoBehaviour
{
    public QuizMusic QuizMusic;
    ...
    private float timeRemaining = 267f; // 5 phút = 300 giây
            ...
            foreach (var key in GlobalResponseData.quests.Keys)
            {
                ...
                if (key == "QuestQuizMusicSO")
                {
                    ...
                    if (quest.currentQuestStepIndex >= 0 && quest.currentQuestStepIndex < quest.questStepStates.Length)
                    {
                        ...
                        {
                            QuizMusic.score = stateValue;
                            Debug.Log($"QuizA1.score updated to: {QuizMusic.score} based on QuestStepState.state");
                        }
                        else
                        {
 ...
 //   private bool questStarted = false;
    ...
    void YourMethod()
    {
        ...
        {
            QuizA2Slider.value = QuizMusic.GetScore();
            QuizA2QuestLog.text = QuizMusic.GetScore().ToString() + "/8";
        }
    }
    ...
    private void QuestStateChange(Quest quest)
    {
        if (quest.info.id == questInfoForJames.id)
        {
            ...
            else if (quest.state == QuestState.FAILED)
            {
                ...
                uiFailedTimerUI.SetActive(true);
                questCompleted = false;
                QuizMusic.RestartGame();   
                QuizMusic.isTimerRunning = false;
