...
public class NPCDaThan : TalkInteraction, IDialogueHandler
{
    [Header("NPC infos")]
    ...
    public override void Interact()
    {
        ...
        //if (!string.IsNullOrEmpty(_nextGroupOnNextInteract))
        //{
        //    SwitchDialogueGroupInstance(_nextGroupOnNextInteract); 
        //    _nextGroupOnNextInteract = null;                       
        //    return;
        //}
...

        {
            ...
            {
                if (groupEntry.Key.GroupName == "DaThan")
                {
                ...
                QuestManager.instance.questMap[questConversation.id].info.questType != QuestInfoSO.QuestType.Quiz)
        {
            ...
            foreach (var groupEntry in container.DialogueGroups)
            {
                if (groupEntry.Key.GroupName == "DaThanStep")
                {
                    afterFinishGroup = groupEntry.Key;
                    // Tìm dialogue khởi đầu trong group này
                    foreach (var dialogue in groupEntry.Value)
    ...
    private string _nextGroupOnNextInteract;

    // Gọi hàm này để set group cho lần tương tác kế tiếp
    public void SetNextGroupOnNextInteract(string groupName)
    {
        _nextGroupOnNextInteract = groupName;

        // Preload dsDialogue để lần sau Interact không bị DaThanStep ghi đè
        SwitchDialogueGroup(groupName);  // chỉ set dsDialogue, KHÔNG mở hội thoại
    }
